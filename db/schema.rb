...
ActiveRecord::Schema[7.1].define(version: 20_250_903_134_805) do
  ...
  create_table 'daily_lists', force: :cascade do |t|
    ...
    t.index %w[date list_type sequence_number], name: 'idx_daily_lists_unique', unique: true
    t.index %w[date list_type status], name: 'idx_daily_lists_search'
  end
  ...
  create_table 'presences', force: :cascade do |t|
    ...
    t.datetime 'created_at', null: false
    t.datetime 'updated_at', null: false
    t.index %w[daily_list_id position], name: 'idx_presences_unique_position', unique: true,
                                        where: "((status)::text = 'confirmed'::text)"
    t.index %w[daily_list_id status], name: 'idx_presences_list_status'
    t.index %w[daily_list_id user_id], name: 'idx_presences_unique_user', unique: true,
                                       where: "((status)::text = 'confirmed'::text)"
    t.index ['daily_list_id'], name: 'index_presences_on_daily_list_id'
    t.index %w[user_id status], name: 'idx_presences_user_status'
  ...
  create_table 'users', force: :cascade do |t|
    ...
    t.index %w[name phone], name: 'index_users_on_name_and_phone', unique: true
