class Auth::RegisterOperation < ApplicationOperation
  ...
  def validate_required_fields!
    ...
    required_fields.each do |field|
      raise StandardError, "#{field.to_s.humanize} is required" if @parsed_params[field].blank?
    end
  end
  ...
  def validate_nickname_format!
    ...
    raise StandardError, 'Nickname must be at least 2 characters long' if nickname.length < 2
    raise StandardError, 'Nickname cannot be longer than 20 characters' if nickname.length > 20
    return if nickname =~ /\A[a-zA-Z0-9_-]+\z/
  ...
  def validate_rank_combination!
    ...
    if medal == 'immortal'
      unless stars.is_a?(Integer) && stars.positive?
        raise StandardError, 'Immortal rank must have a positive number for MMR ranking'
      end
    else
      raise StandardError, 'Rank stars must be between 1 and 5' unless stars.is_a?(Integer) && stars.in?(1..5)
    end
  end
  ...
  def validate_positions_array!
    ...
    raise StandardError,
          "Invalid positions: #{invalid_positions.join(', ')}. Valid positions: #{User::POSITIONS.join(', ')}"
  end
  ...
  def check_email_availability!
    return unless User.exists?(email: @parsed_params[:email])

    raise StandardError, 'Email is already registered'
  end

  def check_name_phone_combination!
    return unless User.exists?(name: @parsed_params[:name], phone: @parsed_params[:phone])

    raise StandardError, 'Name and phone combination already exists'
  end

  def check_phone_availability!
    return unless User.exists?(phone: @parsed_params[:phone])
